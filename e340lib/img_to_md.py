...
def mdimg_to_imgtag(para):
    for (match, tag, link, opts) in re_mdimg.findall(para):
        ...
        if tag:
            para = para.replace(
                match, '<img src="%s"%s>%s</img>' % (link, style_opts, tag)
            )
        else:
            para = para.replace(match, '<img src="%s"%s />' % (link, style_opts))
    return para
...
def imgtag_to_mdimg(para):
    ...
    for (match, opts, tag) in re_imgtag.findall(para):
        ...
        para = para.replace(match, "![%s](%s){%s}" % (tag, l.group(1), style_opts))
    return para


def find_img(para):
    """
    find img tags like # <img src="./media/image1.png" style="width:5.27in;height:4.0in" />
